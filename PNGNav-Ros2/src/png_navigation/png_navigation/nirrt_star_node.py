...
from png_navigation.tools.path_planning_classes.nirrt_star_png_2d import get_path_planner
...
class NIRRTStarNode(Node):
    def __init__(
        self,
    ):
        super().__init__('png_navigation_nirrt_star_node')
        self.config = Config()
...
def main(args=None):
    ...
    nirrtsn = NIRRTStarNode()
    try:
        rclpy.spin(nirrtsn)
    except KeyboardInterrupt:
        nirrtsn.get_logger().info('Node stopped cleanly')
    except Exception as e:
        nirrtsn.get_logger().error(f'Exception in node: {e}')
    finally:
        nirrtsn.destroy_node()
        rclpy.shutdown()
