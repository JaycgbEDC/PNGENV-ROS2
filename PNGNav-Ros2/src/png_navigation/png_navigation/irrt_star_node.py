...
from png_navigation.tools.path_planning_classes.irrt_star_2d import get_path_planner
...
class IRRTStarNode(Node):
    def __init__(
        ...
        self.se_service = self.create_service(SetEnv, 'png_navigation/set_env_2d', self.set_env)
        self.gp_service = self.create_service(GetGlobalPlan, 'png_navigation/get_global_plan', self.get_global_plan)
    
    def get_global_plan(self, request, response):
        response.plan_response = request.plan_request
...
def main(args=None):
    ...
    try:
        ...
        irrtsn.get_logger().info('Node stopped cleanly')
    except Exception as e:
        irrtsn.get_logger().error(f'Exception in node: {e}')
    finally:
        irrtsn.destroy_node()
        rclpy.shutdown()
