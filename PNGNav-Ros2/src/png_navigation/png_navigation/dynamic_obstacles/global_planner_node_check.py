...
class GlobalPlanner(Node): 
    def __init__(
        ...
        self.create_service(GlobalReplan, 'png_navigation/global_replan', self.global_replan)
    
    def global_replan(self, request, response):
        human_obstacles = request.replan_human_obstacles
        if len(human_obstacles) == 0:
            self.get_logger().info("replan with no human neighbors.")
            self.plan()
            response.is_replanned = self.is_replanned
            return response
        human_obstacles = np.array(human_obstacles).reshape(-1,3)
        env_with_humans = copy.deepcopy(self.env)
        if env_with_humans.obs_circle is None or len(env_with_humans.obs_circle) == 0:
            env_with_humans.obs_circle = human_obstacles
        else:
            env_with_humans.obs_circle = np.concatenate(
                (np.array(env_with_humans.obs_circle), human_obstacles),
                axis=0,
            ) # (n+m, 3)
        self.set_environment(env=env_with_humans)
        if self.use_neural_wrapper:
            self.set_environment_for_neural_wrapper(env=env_with_humans)
        self.get_logger().info("replan")
        self.plan()
        self.set_environment(env=None)
        if self.use_neural_wrapper:
            self.set_environment_for_neural_wrapper(env=None)
        response.is_replanned = self.is_replanned
        return response
    ...
    def set_environment(self, env=None):
        ...
        request_env = SetEnv.Request()
        request_env.request_env.num_dimensions = 2
        request_env.request_env.x_range = tuple(np.array(env.x_range, dtype=np.float64))
        request_env.request_env.y_range = tuple(np.array(env.y_range, dtype=np.float64))
        if env.obs_circle is None:
            request_env.request_env.circle_obstacles = []
        else:
            request_env.request_env.circle_obstacles = tuple(np.array(env.obs_circle, dtype=np.float64).flatten())
        if env.obs_rectangle is None:
            request_env.request_env.rectangle_obstacles = []
        else:
            request_env.request_env.rectangle_obstacles = tuple(np.array(env.obs_rectangle, dtype=np.float64).flatten())
        self.se_client.call_async(request_env).add_done_callback(self.set_environment_callback)
    
    def set_environment_for_neural_wrapper(self, env=None):
        if env is None:
            env = self.env
        ...
        if env.obs_rectangle is None:
            ...
            request_env.request_env.rectangle_obstacles = tuple(np.array(env.obs_rectangle, dtype=np.float64).flatten())
        self.sen_client.call_async(request_env).add_done_callback(self.set_environment_for_neural_wrapper_callback)

    def set_environment_callback(self, future):
        if future.result().is_set:
            self.get_logger().info('Environment set successfully.')
        else:
            self.get_logger().error('Service call failed')

    def set_environment_for_neural_wrapper_callback(self, future):
    ...
    def plan_callback(self, future):
        ...
        self.is_replanned = future.result().is_solved
    ...
    def plan(self):
        ...
        plan_request.plan_request.max_time = np.float64(self.config.path_planner_args.max_time)
...
def main(args=None):
    ...
    rclpy.logging.get_logger('global_planner_node_check:main').info("Global Planner is initialized.")
    try:
        rclpy.spin(gp)
