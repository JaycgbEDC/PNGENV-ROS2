...
from png_navigation.tools.wrapper.pointnet_pointnet2.models.pointnet2_utils import pc_normalize
from png_navigation.tools.datasets.point_cloud_mask_utils_updated import get_point_cloud_mask_around_points
from png_navigation.tools.wrapper.pointnet_pointnet2.models.pointnet2_sem_seg_msg_pathplan import get_model
from png_navigation.tools.wrapper.utils.bfs_connect_heuristic import get_boundary_mask, bfs_point_cloud_visualization, select_heuristic_boundary_point
...
class PNGWrapper:
    def __init__(
        ...
        if random_seed is None:
            model_filepath = join(root_dir, 'model_weights/pointnet2_sem_seg_msg_pathplan.pth')
        else:
            # 100, 200, 300, 400, 500
            model_filepath = join(root_dir, 'model_weights/pointnet2_sem_seg_msg_pathplan_'+str(random_seed)+'.pth')
        ...
        if random_seed is None:
            ...
            print("PointNet++ wrapper trained with random seed "+str(random_seed)+" is initialized. Using BFS for Neural Connect.")
    ...
    def generate_connected_path_points(
        ...
        for trial_i in range(max_trial_attempts):
            ...
            print("inference time: {0:.2f}", time.time()-inference_ts)
