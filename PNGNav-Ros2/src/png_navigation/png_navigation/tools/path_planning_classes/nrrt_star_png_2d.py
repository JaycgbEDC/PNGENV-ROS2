...
from png_navigation.tools.path_planning_classes.rrt_base_2d import RRTBase2D
from png_navigation.tools.path_planning_classes.rrt_star_2d import RRTStar2D
from png_navigation.tools.path_planning_classes.rrt_visualizer_2d import NRRTStarPNGVisualizer

from geometry_msgs.msg import Point
from visualization_msgs.msg import Marker, MarkerArray
from std_msgs.msg import String, Int32, Float64MultiArray

class NRRTStarPNG2D(RRTStar2D, Node):
    def __init__(
        self,
        x_start,
        x_goal,
        ...
        self.planning_start_end_pub = self.create_publisher(String, 'planning_start_end', 10)
        self.path_len_pub = self.create_publisher(Float64MultiArray, 'path_len', 10)
        self.time_pub = self.create_publisher(String, 'time_record', 10)
        self.neural_wrapper_pub = self.create_publisher(NIRRTWrapperMsg, 'wrapper_input', 10)
        self.tree_pub = self.create_publisher(MarkerArray, 'tree', 10)
        self.current_path = None
        self.path_point_cloud_pred = None
        time.sleep(1)
        self.create_subscription(Int32, 'random_timer', self.random_timer_callback, 10)
        self.create_subscription(Float64MultiArray, 'wrapper_output', self.pc_callback, 10)
    ...
    def update_point_cloud(self):
        ...
        msg.c_max = np.float64(0) # dummy
        msg.c_min = np.float64(0) # dummy
